...
namespace Inside.Xamarin.ViewModels
{
    public class CoinPageViewModel : BaseViewModel
    {
        ...
        private async void BuyCoins(object coinsQuantity)
        {
            ...
            var result = await NotificationService.GetInstance().ShowDialogAlertOnMaster(string.Format(Languages.CoinBuyConfirmAlert, coinsQuantity), Languages.GeneralConfirm);
            ...
            if (result)
            {
                ...
                NotificationService.GetInstance().ShowInfoAlertOnMaster(Languages.GeneralCongratulations, string.Format(Languages.CoinBuyCongratulationsAlert, coinsQuantity));
            }
        }
    }
